...
class BookingDetailsDialogFragment : DialogFragment() {
    ...
    private fun setupTimeline(booking: BookingResponse) {
        ...
        steps.add(
            TimelineStep(
                "Payment Confirmed",
                formatDate(booking.createdAt) ?: "Booking is secured.",
                true
            )
        )
        ...
        if (status == "REJECTED" || status == "REJECTED_REFUNDED" || status == "CANCELLED" || status == "CANCELLED_REFUNDED") {
            ...
            steps.add(
                TimelineStep(
                    "Booking Accepted",
                    formatDate(booking.acceptedAt) ?: "Awaiting confirmation...",
                    isAccepted
                )
            )
            ...
            steps.add(
                TimelineStep(
                    "Service Completed",
                    formatDate(booking.completedAt) ?: "Ready for service date.",
                    isCompleted,
                    isLast = true
                )
            )
