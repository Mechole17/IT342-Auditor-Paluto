...
import edu.cit.auditor.paluto.adapter.PortfolioAdapter
import edu.cit.auditor.paluto.adapter.PortfolioItem
...
class CookPortfolioFragment : Fragment() {
    ...
    private fun showManageServiceDialog(service: ServiceResponse?) {
        val dialog = ManageServiceDialogFragment.newInstance(service) {
            fetchData()
        }
    ...
    private fun showUploadCertDialog() {
        val dialog = UploadCertificateDialogFragment.newInstance {
            fetchData()
        }
